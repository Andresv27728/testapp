...
// --- Método 4: Maya API ---
export async function downloadWithMaya(url, isVideo = false) {
  const type = isVideo ? 'mp4' : 'mp3';
  const apikey = 'may-f29f21da8710bfec9e330daaf869a0e6';
  const apiUrl = `https://mayapi.giize.com/ytdl?type=${type}&apikey=${apikey}&url=${encodeURIComponent(url)}`;

  try {
    const response = await axios.get(apiUrl, { timeout: 20000 }); // 20 segundos de timeout
    const result = response.data;

    const downloadUrl = result.url || result.link || result.download || (result.result && result.result.url);

    if (!downloadUrl) {
      console.error("Respuesta inesperada de Maya API:", JSON.stringify(result, null, 2));
      throw new Error("La respuesta de la API no contenía un enlace de descarga válido.");
    }

    return downloadUrl;
  } catch (error) {
    if (error.code === 'ECONNABORTED') {
        throw new Error("Maya API no respondió a tiempo (timeout).");
    }
    console.error(`Error al contactar Maya API: ${error.message}`);
    throw new Error("No se pudo obtener una respuesta válida de la API de descarga.");
  }
}
