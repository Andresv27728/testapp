...
const getDbCommand = {
  ...
  async execute({ sock, msg, args, config }) {
    ...
    if (!fileName) {
      return sock.sendMessage(msg.sender, { text: "Por favor, especifica qué archivo de la base de datos quieres. (ej. `groups.json`)" });
    }
    ...
    if (fs.existsSync(dbPath)) {
      try {
        ...
        await sock.sendMessage(msg.sender, { text: "Ocurrió un error al enviar el archivo." });
      }
    } else {
      await sock.sendMessage(msg.sender, { text: `El archivo de base de datos '${fileName}' no existe.` });
    }
  }
};
